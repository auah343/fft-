...
namespace FFTLibrary {
    ...
    // FFTColumn：按列计算 FFT
    Eigen::MatrixXcd FFTColumn::fft(const Eigen::MatrixXcd& X, int n, int dim) const {
    ...
    // FFTRow：按行计算 FFT
    Eigen::MatrixXcd FFTRow::fft(const Eigen::MatrixXcd& X, int n, int dim) const {
        Eigen::FFT<double> fft;
        Eigen::MatrixXcd Y(X.rows(), X.cols());
